...
function markAllCategoryQuestions(stepIndex, categoryName) {
    ...
    // Show confirmation
    alert(`✅ All ${markedCount} questions in "${categoryName}" have been marked as completed!`);
}
    ...
    steps.forEach((step, stepIndex) => {
        ...
        if (stepCategories.length > 0) {
            stepCategories.forEach(categoryName => {
                ...
                categoryMarkAllBtn.textContent = 'Mark All ✓';
                ...
                categoryHeader.appendChild(categoryMarkAllBtn);
